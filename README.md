...
| `image-warp-clicks` | Offer similar functionarity to `image-warp`.  User can create imageometry file with clicking image and typing stage corrdinates.  This command cannot accept --range and --density options unlike `image-warp`.                                                                                                                                                                                                         | Incompatible with OpenCV3. Supports `affine_xy2vs`. Interactive-command. |
...
| `make_tiles`        | Project an image into VS space and export a squared sub-area of the VS space as map tiles (256x256 pixels images) which can be used with the [Leaflet.js](https://leafletjs.com/) (a Javascript library for interactive maps).  Note that the sides of the source image must be parallel to the VS coordinate axis because this program does not rotate the image. Location to project the image is set by x or y coordinates of four (left, upper, right, and bottom) sides of the source image on the VS space. The squared sub-area is specified by the coordinate of the center of the area and the `width` of the area on the VS space. The number of map tiles and resolution of each map tile depend on zoom levels. The minimun zoom level is 0. At the zoom level 0 the squared sub-area is projected to a 256x256 square image. Thus the number and resolution of map tile is 1 and 256/`width`, respectively. With icreasing zoom level the squared image width is multiplied by 2, thus the suqared sub-area is projected to a 512x512 image with 512/`width` resolution and at zoom level 2 the squared image is a 1024x1024 pixels image. At each zoom  at zoom level 1), thus the squared images  512x512 squared image is 4 tiles with 512/`length` resolution are generated at zoom level 1, 16 tiles with 1024/`length` resolution are generated at zoom level 2, and so on. This program generates map tiles for each zoom level in a directory with the following structure: {zoom level}/{x}_{y}.png. For example at zoom level 2, the 16 tiles are exported as 2/0_0.png, 2/1_0.png, 2/2_0.png, ..., and 2/3_3.png. The maximum zoom level is specified by arguments.| [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
...
| `haffine_from_points` | Same as `h_from_points` but takes three pairs of coordinates instead of four.                                                                                                           | Compatible with OpenCV3.                                                                                                                                                                                                                                                                                                                                                     |
...
| `haffine_from_params` | Return Affine matrix calculated from center of rotation in original image, rotation angle, and magnification. Parameters should be fed by arguments. | Compatible with OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool).                                                                                                                                                                                                                                                         |
