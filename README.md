...
| `make_tiles`        | Project an image into VS space and export a squared sub-area of the VS space as mosaic.  The mosaic onsists of tiles of image with 256x256 pixels.   Note that the edges of the source image must be parallel to the VS coordinate axis because this program does not support projection with rotation. Location to project the source image is set by x or y coordinate of four edges (x coordinate of left and right edges and y coordinate of upper and bottom edges). The squared sub-area on the VS space is specified by center and width. The number of tiles depends on `zoom levels`.  At `zoom level` 0 the squared sub-area is exported as a tile. Thus resolution of a tile is 256/`width` (pixel/micron). With increment of zoom level the number of exporting tiles is multiplied by 2x2.   Number and resolution of tiles  are 4  and 512/`width` (pixel/micron) at zoom level 1 and 16  and 1024/`width` (pixel/micron) at zoom level 2.  This program generates a serires of tiles for zoom level from 0 to `max`. The zoom level `max` is specified by arguments.  The tiles are compatible with [Leaflet.js](https://leafletjs.com/) (a Javascript library for interactive maps). The tiles are exported as {zoom level}/{x}_{y}.png where x and y correspond to n-th coordinate of tile in horizontal and vertical direction.  For example at zoom level 2,  16 tiles are exported as 2/0_0.png, 2/1_0.png, 2/2_0.png, ..., and 2/3_3.png.| Used in [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
