...
| `warp_image`        | Transform and export image.  Affine matrix can be also specified by center of ratation, angle of rotation, and magnification as similar to `haffine_from_params`. The area to exported is specified by width and height via arguments.  Note that the top-left corner of the image is located on the origin.  Without width and height specified, those of  source image would be applied.  This program also offers similar functionarity of `image_in_image`.                                        | Compatible with OpenCV3. Supports affine_ij2ij. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
