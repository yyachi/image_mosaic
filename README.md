...
| `warp_image`        | Perform geometrical transformation of an image and export the image. The Affine matrix can be also specified by center of ratation, angle of rotation, and magnification as similar to `haffine_from_params`. The size of the output image can be specified as an argument.  This command supposes the top-left corner of the image to be the origin or supports affine_ij2ij.  After projection, area specified by width and height with top-left corner to be located on the origin would be exported.  Without width and height specified, width and height of original image would be specified.  This program also offers similar functionarity of `image_in_image`.                                        | Compatible with OpenCV3. Supports affine_ij2ij. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
