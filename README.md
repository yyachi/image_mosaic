...
| `make_tiles`        | Project an image into VS space and export a square area of the VS space as map tiles which can be used with the [Leaflet.js](https://leafletjs.com/) mapping platform.  Note that the image must be parallel to the VS coordinate axis because this program does not rotate the image. Location to project the image is set by locations of four (left, upper, right, and bottom) sides of the image on the VS space. The square area is specified by the coordinate of the center of the area and the length of the area on the VS space (`length`). On a Leaflet map, the minimun zoom level (the square area on a single tile) is 0. This corresponds to a 256x256 square image. With each zoom level the square image size is multiplied by 2, thus at zoom level 1 the square area is projected to a 512x512 square image with 512/(the length of the area on the VS space) resolution. Then the square image is split into 4 map tiles (256x256 pixels images) and exported as 1/0_0.png, 1/0_1.png, 1/1_0.png, and 1/1_1.png. For example at zoom level 2, the program projects the source image into the VS space, and split the square area (a 1024x1024 square image) into 16 map tiles and export them as 2/0_0.png, 2/1_0.png, 2/2_0.png, ... 2/3_3.png. The maximum zoom level is specified by arguments.  This program generates map tiles (256x256 pixles image) for zoom levels in a directory with the following structure: {zoom level}/{x}_{y}.png. The resolution of map tiles for a zoom level is set by length of the square area multiplied by zoom level. | [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
