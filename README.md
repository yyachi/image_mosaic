...
| `make_tiles`        | Project an image into VS space and export a squared sub-area of the VS space as tile images (256x256 pixel images) which can be used with the [Leaflet.js](https://leafletjs.com/) (a Javascript library for interactive maps).  Note that the edges of the source image must be parallel to the VS coordinate axis because this program does not support projection with rotation. Location to project the source image is set by x or y coordinates of four edges (x for left and right edges and y for upper and bottom edges) of the projected image on the VS space. The squared sub-area is specified by the coordinate of the center of the area and the `width` of the area on the VS space. The number of tile images and resolution of each tile image depend on zoom levels. At the zoom level 0 (minimum) the squared sub-area is exported as a 256x256 pixel image. Thus the number and resolution of map tile is 1 tile and 256/`width`, respectively. With increasing zoom level the width of squared image is multiplied by 2 (zoom level 1: a 512x512 pixel image with resolution 512/`length`, zoom level 2: a 1024x1024 pixel image with 1024/`width` resolution, ..., zoom level n: a (256x2^n)x(256x2^n) pixel image with 256x2^n/`length` resolution). The projected suqare image for each zoom level is split as tile images (256x256 pixel image). Thus the number of tile images are 4 tiles at  zoom level 1, 16 tiles at zoom level 2, ..., and 2x2^`n` tiles at zoom level `n`. This program generates tile images for each zoom level (from zoom level 0 to max zoom level specified by arguments) and exports tile images in a directory with the following file path: {zoom level}/{x}_{y}.png. For example at zoom level 2, the 16 tiles are exported as 2/0_0.png, 2/1_0.png, 2/2_0.png, ..., and 2/3_3.png.| Used in [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
