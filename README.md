...
| command             | description                                                                                                                                                                          | note                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------                                                                                               | ----                                                                                                                                                                                                                                                                                                                                                                                               |
| `warp_image`        | Transform an image using Affine matrix `affine_ij2ij` and export image.  Affine matrix can be also specified by center of ratation, angle of rotation, and magnification as similar to `haffine_from_params`. The area to be exported is specified by width and height via arguments.  Note that the top-left corner of the image is located on the origin.  Without width and height specified, those of source image would be applied. This program also impose the image on wall image after Affine transformation.  In this case, Affine matrix is calculated from coordinates of 4 corners of the image and those projected onto the wall image specified by arguments.| Incompatible with OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
...
| `make_tiles`        | Make an image zoomable with Leaflet.js. This program generates map tiles (256x256 pixels images) in a direcotory with the following structure: {zoom level}/{x}_{y}.png. The minimum zoom level is 0. This correspond to a 256x256 square image and can be represented by a single tile image (0/0_0.png).  The program transforms the source image to fit with the 256x256 square image. With each zoom level the square size is multiplied by 2. The default maximum zoom level is 2. This corresponds to a 1024x1024 square image and can be represented by 16 tile images (2/0_0.png, 2/1_0.png, 2/2_0.png, ... 2/3_3.png). The program transforms the source image to fit with the 1024x1024 square image and split it in 16 tile images. The maximum zoom level can be specified by arguments.| [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
