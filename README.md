...
| command             | description                                                                                                                                                                          | note                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------                                                                                               | ----                                                                                                                                                                                                                                                                                                                                                                                               |
| `warp_image`        | Transform an image using Affine matrix `affine_ij2ij` and export image.  Affine matrix can be also specified by center of ratation, angle of rotation, and magnification as similar to `haffine_from_params`. The area to be exported is specified by width and height via arguments.  Note that the top-left corner of the original image is the origin.  Without width and height specified, those of source image would be applied. This program also impose the image on wall image after Affine transformation.  In this case, Affine matrix is calculated from coordinates of 4 corners of the image and those projected onto the wall image specified by arguments.| Incompatible with OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
