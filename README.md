...
| haffine_from_params | Return Affine matrix calculated from parameters. The parameters are the center of the rotation in the source image, rotation angle and Isotropic scale factor (magnification). The parameters are fed by arguments. | Compatible with OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool). |
