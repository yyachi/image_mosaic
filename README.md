...
| `make_tiles`        | Project an image into VS space and export a square area of the VS space as map tiles which can be used with the [Leaflet.js](https://leafletjs.com/) (a Javascript library for interactive maps).  Note that the image must be parallel to the VS coordinate axis because this program does not rotate the image. Location to project the image is set by locations of four (left, upper, right, and bottom) sides of the image on the VS space. The square area is specified by the coordinate of the center of the area and the `length` of the area on the VS space. The number and resolution of map tiles depend on zoom levels. On a Leaflet map, the minimun zoom level is 0. At the zoom level 0 the square area is projected to a 256x256 square image. Thus the number and resolution of map tiles are 1 and 256/`length`, respectively. With each zoom level the square image size is multiplied by 2, thus 4 tiles with 512/`length` resolution are generated at zoom level 1, 16 tiles with 1024/`length` resolution are generated at zoom level 2, and so on. This program generates map tiles for each zoom level in a directory with the following structure: {zoom level}/{x}_{y}.png. For example at zoom level 2, the 16 tiles are exported as 2/0_0.png, 2/1_0.png, 2/2_0.png, ..., and 2/3_3.png. The maximum zoom level is specified by arguments.| [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
