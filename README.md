...
| `make_tiles`        | Project an image into VS space and export an square area of the VS space as map tiles (256x256 pixels images). Location to project the image is set by VS coordinates of 4 sides (left, upper, right, bottom) of the image. Note that the image must be parallel to the VS coordinate axis because this program does not rotate the image. The square area exported as map tiles is specified by cordinate of the center and length of the square. This program generates map tiles (256x256 pixels images) in a directory with the following structure: {zoom level}/{x}_{y}.png. For example at zoom level 2, the program projects the source image into the VS space (1024x1024 square image) and split it into 16 map tiles (2/0_0.png, 2/1_0.png, 2/2_0.png, ... 2/3_3.png). The region of the sub-area is set by length and center. Affine matrix is calculated from coordinates where the 4 corners of the source image are projected. The maximum zoom level is specified by arguments.| [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |
