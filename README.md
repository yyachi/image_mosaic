...
| `make_tiles`        | Project an image into VS space and export a squared sub-area of the VS space as tile images (256x256 pixel images) which can be used with the [Leaflet.js](https://leafletjs.com/) (a Javascript library for interactive maps).  Note that the edges of the source image must be parallel to the VS coordinate axis because this program does not support projection with rotation. Location to project the source image is set by x or y coordinates of four edges (x coordinates of left and right edges and y coordinates of upper and bottom edges) of the source image on the VS space. The squared sub-area is specified by the coordinate of the center of the area and the `width` of the area on the VS space. The number of tile images and resolution of each tile image depend on zoom levels. At the zoom level 0 the squared sub-area is exported as a 256x256 pixel image. Thus the number and resolution of map tile is 1 tile and 256/`width`, respectively. With increasing zoom levels the width of exported image is multiplied by 2 (zoom level 1: a 512x512 pixel image, zoom level 2: a 1024x1024 pixel image, ..., zoom level n: a (256x2^n)x(256x2^n) pixel image). The exported images for each zoom levels are divided into tile images (256x256 pixel images). Thus the number of tile images and the resolution of each tile image are 4 tiles and 512/`width` resolution at zoom level 1, 16 tiles and 1024/`width` at zoom level 2, ..., and 4^`n` tiles and (256x2^n)/`width` at zoom level `n`. This program generates tile images from zoom level 0 to zoom level `max`. The `max` zoom level is specified by arguments. The tile images for each zoom levels are exported in a directory with the following file path: {zoom level}/{x}_{y}.png. For example at zoom level 2, the 16 tiles are exported as 2/0_0.png, 2/1_0.png, 2/2_0.png, ..., and 2/3_3.png.| Ready for OpenCV3. Used in [rails project -- medusa](https://github.com/misasa/medusa)                                                                                                                                                                                                                                                                                 |

Commands to calculate a transform matrix are shown below.

| command               | description                                                                                                                                                                                | note                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------   | --------------------------------------------------------------------------------------                                                                                                     | ----                                                                                                                                                                                                                                                                                                                                                                         |
| `h_from_points`       | Return Affine matrix calculated from four pairs of coordinates.  Coordinates should be fed by arguments.                                                                                     | Ready for OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage). |
| `haffine_from_points` | Same as `h_from_points` but takes three pairs of coordinates instead of four.                                                                                                           | Ready for OpenCV3.                                                                                                                                                                                                                                                                                                                                                     |
| `affine_from_points`  | Same as `haffine_from_points` but coordinates should be fed by stdin instead of by arguments.                                                                                              | Ready for OpenCV3.|
| `haffine_from_params` | Return Affine matrix calculated from center of rotation in original image, rotation angle, and magnification. Parameters should be fed by arguments. | Ready for OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool).                                                                                                                                                                                                                                                         |

Commands to apply geometrical transform to an image or points are shown below.

| command             | description                                                                                                                                                                          | note                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------                                                                                               | ----                                                                                                                                                                                                                                                                                                                                                                                               |
| `warp_image`        | Transform an image using Affine matrix `affine_ij2ij` and export image.  Affine matrix can be specified by (1) 3x3 matrix, (2) center of rotation in original image, rotation angle, and magnification as similar to `haffine_from_params`, and (3) coordinates where the 4 corners of the original image are transformed. The area to be exported can be specified by width and height via arguments. Without width and height specified, those of the original image would be applied. This program also imposes the transformed image on wall image. In this case, the area to be exported is set by width and height of the wall image. | Incompatible with OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
| `image_in_image`    | Impose an image on wall image after Affine transformation.  Affine matrix `affine_ij2ij` is calculated from coordinates of 4 corners of the image and those projected onto the wall image specified by arguments. This command is subset of `warp_image`. | Ready for OpenCV3. Used in [rails project -- medusa](https://github.com/misasa/medusa).                                                                                                                                                                                                                                                                                                      |
| `transform_image` (obsolete)  | This command is subset of `warp_image`.  The area to be exported is set by width and height  of the source image.                                                                  | Ready for OpenCV3.|
| `transform_points`  | Return coordinates after Affine transformation.  Original coordinates and Affine matrix are specified by arguments.                                                                                                                             | Ready for OpenCV3. Used in [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool), [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage)                        |

Other commands are shown below.

| command             | description                                                                            | note                                                                                                                                                                                                                                                                                                                                        |
| ------------------- | -------------------------------------------------------------------------------------- | ----                                                                                                                                                                                                                                                                                                                                        |
| `blend-image`       | Impose an image to wall image with alpha blend techniques.  Location to impose the image is set by ij corrdinates via arguments.                                        | Ready for OpenCV3. |
| `crop_image`        | Crop a rectangular region of an image.                                              | Ready for OpenCV3. [gem package -- opencvtool](https://gitlab.misasa.okayama-u.ac.jp/gems/opencvtool), [gem package -- vstool](https://gitlab.misasa.okayama-u.ac.jp/gems/vstool),  [rake project -- mosaic-sem](https://gitlab.misasa.okayama-u.ac.jp/DREAM/mosaic-sem), [gem package -- multi_stage](https://gitlab.misasa.okayama-u.ac.jp/gems/multi_stage) |
