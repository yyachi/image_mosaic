...
class ViewWindow:
	...
	def set_stage_geometry(self, x_range, y_range, pixel_per_um = None):
		...
		if pixel_per_um != None:
			self.output_pixel_per_um = pixel_per_um
			self.output_size = tuple([int(math.ceil(float(stage_width) * pixel_per_um)), int(math.ceil(float(stage_height) * pixel_per_um))])
	...
	def set_output_size_from_aspect(self, verbose = False):
		...
		self.output_pixel_per_um = float(output_image_width_height[0]) / self.stage_width
	...
	def warp_image(self, affine, view_geometry, verbose = False):
		...
		image_rect = [(0,0),(width,0),(width,height),(0,height)]
		image_rect_on_stage = transform_points(image_rect, affine)
		self.original_pixel_per_um = float(width) / abs(image_rect_on_stage[0][0] - image_rect_on_stage[1][0])
		...
		if verbose:
			...
			print "original pixel_per_um: %.3f" % (self.original_pixel_per_um)
			print "output image: %dx%d (%.3f)" % (self.output_size[0], self.output_size[1], float(self.output_size[0])/self.output_size[1])
			print "output pixel_per_um: %.3f" % (self.output_pixel_per_um)
	...
	def resize_and_save(self, filepath, pixel_per_um):
		self.save_size = (int(self.view_image.width * pixel_per_um), int(self.view_image.height * pixel_per_um))
...
def get_default_view_width_height(affine, width, height):
	...
	pixel_per_um = float(output_image_width_height[0]) / stage_width
	...
	print "pixel_per_um: %.3f" % (pixel_per_um)
...
""")
	...
	win_view.set_stage_geometry(x_range = [-5596, 4244], y_range = [-11715, -9196], pixel_per_um = 1.0)
	...
	win_view.save_output(default_ofile_name)
	...
	print affine_to_str(affine_warped)
	...
	#default_pixel_per_um = float(original_width) / default_view[0]
	#r_h = float(original_height) / default_view[1]

	#if default_pixel_per_um > r_h:
	#	default_pixel_per_um = r_h
	#win_view.resize_and_save(default_ofile_name, default_pixel_per_um)
	...
	#print "resolution: %.3f pix/um" % (default_pixel_per_um)
